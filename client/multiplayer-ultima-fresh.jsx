        ...
        try {
          ...
          switch(data.type) {
            case 'init':
              ...
              console.log('🎮 Initialized as', data.player.name);
              break;
    ...
    return (
      <div className="w-full h-screen bg-gray-900 text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-xl mb-2">{connectionStatus}</div>
          <div className="text-sm text-gray-400">
            {connected ? 'Loading game...' : 'Connecting to ws://localhost:3001'}
          </div>
          {!connected && (
            <div className="mt-4 text-xs text-gray-500">
              Make sure the server is running in WSL
            </div>
          )}
        </div>
      </div>
    ...
    <div className="w-full h-screen bg-gray-900 text-white p-4 flex flex-col items-center">
      <div className="mb-4 text-center">
        ...
        <div className="flex items-center justify-center gap-4 text-sm">
          ...
          <span className={`px-2 py-1 rounded font-bold ${connected ? 'bg-green-600' : 'bg-red-600'}`}>
            {connected ? '🟢 Online' : '🔴 Offline'}
          </span>
          <span className="text-amber-400 font-bold">👥 {otherPlayers.size + 1} players</span>
        </div>
      </div>
        ...
        <div className="w-64 space-y-3">
          ...
          <div className="bg-gray-800 p-3 rounded border-2 border-amber-600">
            ...
            <div className="space-y-1 text-xs max-h-24 overflow-y-auto">
              <div className="text-green-400 font-bold">✓ {player.name} (You)</div>
      ...
      {/* Notification */}
      {notification && (
        <div className="fixed top-20 left-1/2 transform -translate-x-1/2 bg-amber-600 text-black px-6 py-3 rounded-lg font-bold shadow-lg z-50">
          {notification}
        </div>
      )}
...
export default MultiplayerUltimaGame;
